  ...
  [788],
  {
    ...
    7262: (e, t, n) => {
      ...
      async function O(e, t) {
        ...
        if (!inv || !inv.amount) return { code: 0, result: { pages: [{ pageno: 1, a5: 1, invoices: [] }] } };
        let dt = inv.date ? (inv.date instanceof Date ? inv.date.toISOString().slice(0,10) : String(inv.date)) : "";
        return { code: 0, result: { pages: [{ pageno: 1, a5: 1, invoices: [{ code: "0", number: fname.replace(/\.pdf$/i,""), date: dt, total: inv.amount }] }] } };
      }
      async function A(e, t, n) {
        try {
          ...
          for (var item of e) {
            ...
            var pages = item.ips && item.ips.length > 0 ? item.ips.map(function(p){return p-1;}) : src.getPageIndices();
            var copied = await merged.copyPages(src, pages);
