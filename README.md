...
- **新增「适用场景」板块**：以图标卡片形式展示适用人群（中小企业财务 / 会计师事务所 / 代理记账 / 个体户报销人）与典型使用场景（月末报销 / 季度申报 / 金额核对 / 同票多份）。
- **新增「常见问题 FAQ」板块**：6 条常见问题，并配套结构化数据（FAQPage Schema）。
- **新增「支持的电子发票类型」标签区**：覆盖增值税专普票、全电发票、铁路电子客票等。
- **SEO 优化**：首页正文内容扩充，补齐关键词与语义化结构；同步更新 `sitemap.xml`。
- **全终端适配**：新增媒体元素溢出兜底与超小屏（≤380px）断点，覆盖手机 / 平板 / 桌面 / 大屏。

> 首页导航新增「适用场景」「常见问题」锚点入口。
...
本项目源代码在 GitHub 公开**仅供个人学习、研究与交流**，并**未采用**任何
开源许可证。未经著作权人书面授权，**禁止商业使用、复制镜像、二次分发或发布
竞争性衍生产品**。完整条款见 [LICENSE](./LICENSE)。

- 官方站点：<https://lcofo.cn>（免费在线使用全部功能）
- 商业授权 / 合作：请通过官方站点页面的联系方式获取书面授权

> 提示：本工具为纯前端应用，请认准官方域名 **lcofo.cn**，谨防仿冒站点。
